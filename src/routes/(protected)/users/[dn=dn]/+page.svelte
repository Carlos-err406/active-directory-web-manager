<script lang="ts">
	...
	import { b64ToObjectURL } from '$lib/transforms';
	...
	// Reactive statements to unpack user data
	$: ({ user } = data);
	$: ({ jpegPhoto, sAMAccountName } = user);

	$: if (jpegPhoto && browser) {
		src = b64ToObjectURL(jpegPhoto);
	}
